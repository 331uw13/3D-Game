...
vec3 get_horizon_color(float y) {
    ...
    const float blevel =  500.0; // This value controls where bottom color starts.
    y = map(y, max_y, blevel, 1.0, 0.0);
    y = smoothstep(0.0, 1.0, y);
    float noise = fract(cos(dot(vec2(y*200, -y*200), vec2(52.621,67.1262)))*72823.53)/238.0;
    vec3 res = mix(fog.color_top.rgb+noise, fog.color_bottom.rgb+noise, y);
    return res*0.8;
}
